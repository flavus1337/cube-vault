...
import { priceFor, withinBudget } from './price.ts'
...
test('a budget takes the cheapest cards until the money is gone', () => {
  ...
  assert.deepEqual(
    withinBudget(cards, 4).map((c) => c.price_eur),
    [0.5, 1, 2],
  )
})
...
test('a card without a price counts as free and stays in', () => {
  const cards = [{ price_eur: null }, { price_eur: 10 }]
  assert.deepEqual(
    withinBudget(cards, 1).map((c) => c.price_eur),
    [null],
  )
})
